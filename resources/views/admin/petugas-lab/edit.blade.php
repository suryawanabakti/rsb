...
@section('title', 'Edit Petugas Lab')

@section('content')
    <div class="mb-8">
        <a href="{{ route('admin.petugas-labs.index') }}"
            class="text-slate-500 hover:text-slate-700 text-sm mb-4 inline-block">&larr; Kembali</a>
        <h1 class="text-2xl font-bold text-slate-900">Edit Data Petugas Lab</h1>
        <p class="text-slate-500">Silakan perbarui informasi petugas lab berikut.</p>
    </div>

    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-8 max-w-2xl">
        <form action="{{ route('admin.petugas-labs.update', $petugasLab->id) }}" method="POST">
            @csrf
            @method('PUT')

            <div class="mb-6">
                <label for="name" class="block text-sm font-bold text-slate-700 mb-2">Nama Lengkap</label>
                <input type="text" name="name" id="name" value="{{ old('name', $petugasLab->name) }}" required
            ...
            <div class="mb-6">
                ...
                <input type="text" name="username" id="username" value="{{ old('username', $petugasLab->username) }}"
                    required
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <input type="text" name="nrp" id="nrp" value="{{ old('nrp', $petugasLab->nrp) }}"
                ...
                <div>
                    ...
                    <input type="text" name="phone" id="phone" value="{{ old('phone', $petugasLab->phone) }}"
            ...
            <div class="mb-6">
                ...
                <textarea name="address" id="address" rows="3"
                    class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-emerald-500 outline-none transition-all @error('address') border-red-500 @enderror">{{ old('address', $petugasLab->address) }}</textarea>
                @error('address')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
