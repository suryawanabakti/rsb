...
@section('content')
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
            <div class="flex items-center justify-between mb-4">
                ...
                <span class="text-xs font-bold text-slate-400">LAB RESULTS</span>
            </div>
            <h3 class="text-3xl font-black text-emerald-600">{{ $stats['validated_lab_results'] }}</h3>
            <p class="text-sm text-slate-500 mt-1">Lab Tervalidasi</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
            <div class="p-6 border-b border-slate-100 flex items-center justify-between">
                <h3 class="font-bold text-slate-900 text-lg">Hasil Lab Terbaru</h3>
                <a href="{{ route('pimpinan.reports.index', ['type' => 'lab_results']) }}"
                    class="text-sm text-indigo-600 font-semibold hover:underline">Lihat Laporan</a>
            </div>
            <div class="divide-y divide-slate-100">
                @forelse($recentLabResults as $result)
                    <div class="p-6 hover:bg-slate-50 transition-colors">
                        <div class="flex items-center">
                            <div
                                class="h-10 w-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-600 mr-4 font-bold flex-shrink-0">
                            ...
                            <div class="text-right">
                                <span
                                    ...
                                    {{ strtoupper($result->status == 'validated' ? 'VALID' : $result->status) }}
                                </span>
                            </div>
                        </div>
                    </div>
