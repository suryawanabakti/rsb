...
@section('content')
    <div class="mb-8">
        ...
        <h1 class="text-2xl font-bold text-slate-900 mt-2">Input Hasil Pemeriksaan Baru</h1>
        <p class="text-slate-500">Masukkan data hasil pemeriksaan laboratorium</p>
    </div>

    <form action="{{ route('petugas-lab.lab-results.store') }}" method="POST" class="space-y-6" x-data="labForm()">
        @csrf
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
            ...
            <textarea name="notes" rows="3" placeholder="Catatan opsional..."
                class="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 text-sm">{{ old('notes') }}</textarea>
        </div>
        ...
        function labForm() {
            return {
                ...
                async searchPatients() {
                    // Allow empty search to fetch recent
                    try {
